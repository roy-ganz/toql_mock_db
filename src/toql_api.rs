...
use toql::backend::{count::count, delete::delete, insert::insert, load::load, update::update};
...
impl<'a> ToqlApi for MockDb<'a> {
    ...
    async fn load_page<T, B>(
        ...
        query: B,
        page: Page,
    ) -> Result<(Vec<T>, Option<(u64, u64)>), Self::Error>
    where
        T: Load<Self::Row, Self::Error>,
        B: Borrow<Query<T>> + Send + Sync,
        <T as Keyed>::Key: FromRow<Self::Row, Self::Error>,
    ...
    where
        B: Borrow<K> + Send,
        K: Key + ToQuery<<K as Key>::Entity> + Send,
